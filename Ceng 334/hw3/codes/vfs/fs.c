...
struct dentry *pathwalk(char *path) {
  ...
  struct dentry *result = malloc(sizeof(struct dentry));

  if(!strcmp(path, ".") || !strcmp(path, "/"))
  {
      *result = *current_sb->s_root;
      return result;
  }

  struct inode *tmp = current_sb->s_root->d_inode;
  char *tmp_str;
  tmp_str = strtok(path, "/");
  while(tmp_str)
  {
      result->d_name = tmp_str;
      result = tmp->i_op->lookup(tmp, result);
      tmp = result->d_inode;
      tmp_str = strtok(NULL, "/");
  }
