...
// TODO: OPERATOR=
  ...
  for( int i = 0 ; i < _capacity ; i++ )
    for( int j = 0 ; j < 3 ; j++ )
      if( rhs._table[i].entries[j].Active == true )
...
// RESIZE THE TABLE WITH THE NEXT PRIME NUMBER.
  ...
  while(true)
  {
    for( j = 0 ; j < 3 ; j++ )
    {
      if(_table[h].entries[j].Active == false)
      {
        ...
        if(_table[h].entries[j].Deleted == false || _table[h].entries[j].Key == key)
          break;
      }

      else if(_table[h].entries[j].Key == key)
      {
  ...
  _table[f_h].entries[f_j].Key = key;
  _table[f_h].entries[f_j].Value = value;
  _table[f_h].entries[f_j].Active = true;
  _table[f_h].entries[f_j].Deleted = false;
  _size++;
  total_probes += tmp_tp + 1;
...
// HINT: YOU SHOULD UPDATE ACTIVE & DELETED FIELDS OF THE DELETED ENTRY.
  ...
  while(true)
  {
    for( int j = 0 ; j < 3 ; j++ )
    {
      ...
      if(_table[h].entries[j].Key == key)
      {
        if(_table[h].entries[j].Active == true)
        {
          _size--;
...
// TODO: IMPLEMENT THIS FUNCTION.
// RETURNS THE AVERAGE NUMBER OF PROBES FOR SUCCESSFUL SEARCH
  return (_size == 0)?1.0:(double)total_probes/(double)_size;
}
