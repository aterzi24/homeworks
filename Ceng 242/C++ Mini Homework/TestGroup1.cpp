...
int main() {
    ...
    /************************ Compare book ISBNs ************************/
    // Operator <
    if (*books[0] < 150) cout<<"yes ";    // 120 < 150 ?
    if (*books[0] < 100) cout<<"no ";    // 120 < 100 ?

    // Operator >
    if (*books[3] > 160) cout<<"yes ";    // 170 > 160 ?
    if (*books[3] > 180) cout<<"no ";    // 170 > 180 ?

    // Operator < , comparison with another book
    if (*books[1] < *books[2]) cout<<"yes ";    // 130 < 150 ?
    if (*books[2] < *books[1]) cout<<"no ";    // 150 < 130 ?

    // Check equality
    if (*books[2] < 150) cout<<"no ";    // 150 < 150 ?
    if (*books[2] > 150) cout<<"no ";    // 150 < 150 ?

    HardCopy temp(150, 10.0);
    if (*books[2] < temp) cout<<"no ";   // 150 < 150 ?

    /************************** Discount() Tests **************************/

    // HardCopy discount
    books[5]->Discount();   // new price: 22.95
    cout<<(books[5]->GetPrice())<<endl;
    cout<<"Answer:22.95\n";

    // SoftCopy discount
    books[6]->Discount();   // new price: 7.875
    cout<<(books[6]->GetPrice())<<endl;
    cout<<"Answer:7.875\n";
